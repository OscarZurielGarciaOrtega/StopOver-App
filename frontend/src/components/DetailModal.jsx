...
export default function DetailModal({ isOpen, onClose, rutaId, onCancelar }) {
  ...
  // Cada vez que se abre el modal, buscamos la ruta exacta en localStorage
  useEffect(() => {
    if (isOpen && rutaId) {
  ...
  // 🧮 Lógica para separar las paradas (si hay varias separadas por coma)
  let paradas = [];
              ...
              {paradas.length === 0 ? (
                ...
                paradas.map((parada, index) => (
                  <div key={index} className="relative">
                    ...
                    <div className={`absolute -left-[33px] top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border-2 ${isDarkMode ? 'bg-gray-800 border-emerald-500' : 'bg-white border-[#4F7959]'}`}></div>
                    ...
                    <div className={`p-4 rounded-2xl border transition-colors ${isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-[#FAF9F6] border-gray-100'}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Parada de descanso</p>
                        </div>
                      </div>
                    </div>
                  </div>
